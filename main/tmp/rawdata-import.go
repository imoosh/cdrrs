...
import (
	"bufio"
	"centnet-cdrrs/adapter/file/tmp"
...
func mock1(file string) {
	...
	fw1, err := os.Create("./" + .txt")
	if err != nil {
		log.Debug(err)
		return
	}
	defer fw1.Close()
	log.Debugf("open %s success", conf.InvitePath+"/"+file+"invite.txt")
	...
	fw2, _ := os.Create(conf.ByePath + "/" + file + "bye.txt")
	if err != nil {
		log.Debug(err)
		return
	}
	defer fw2.Close()
	log.Debugf("open %s success", conf.ByePath+"/"+file+"bye.txt")
	...
	for i := 0; i < maxSize; i++ {
		...
		bufWriter2.WriteString(rawByeMessage + "\n")
	}
	bufWriter2.Flush()
