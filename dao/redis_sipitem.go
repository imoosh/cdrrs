...
func (d *Dao) CacheSipItem(data *model.SipCache) {
	var (
...
func (d *Dao) CacheSipItems(data map[string]*model.SipCache) {
	var (
		conn = d.redis.Get(context.TODO())
		args = redis.Args{}
		err  error
...
func (d *Dao) GetSipItem(k string) (item *model.SipCache, err error) {
	var (
		bs   []byte
		conn = d.redis.Get(context.TODO())
	)
...
func (d *Dao) GetSipItems(ids []string) (res map[string]*model.SipCache, err error) {
	var (
		bss  [][]byte
		args = redis.Args{}
		conn = d.redis.Get(context.TODO())
	...
	res = make(map[string]*model.SipCache, len(bss))
	for i, bs := range bss {
		if bs == nil {
			res[ids[i]] = nil
			continue
